...
- `POST /game/dungeon/engage`：发起实时副本 Boss 战斗
- `POST /game/dungeon/commit`：提交实时副本战斗结算（按最终HP）
...
- 修复实时战斗结算：以玩家/怪物最终HP判定胜负，避免“阵亡却通关”
- 修复野外Boss掉落池：不再掉落副本套装装备
- 副本界面改为可滚动，解决小屏设备显示不全
- 副本结算改为服务端校验事件选择并重算加成，必须推进到Boss房后才可结算
- 副本通关判定改为确定性规则（不再使用随机掷点），减少“随机输赢”感
- 每次功能更新都同步维护本 README
